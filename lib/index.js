    ...
    async function catalog(providerId) {
        ...
        for (const pid of providerIds) {
            ...
            if (models.length === 0) {
                ...
                console.warn(`[subagent-model-picker] listModels("${pid}") empty; declared fallback=${fallback.length} (configured=${JSON.stringify((llm.listConfigurableProviders?.() ?? []).map((c) => c?.provider ?? c?.id))})`);
                models = fallback;
            }
