...
export interface Config {
  ...
  modelPool?: Array<{ provider: string; model: string }>
}
...
function parentRoute(agent: any): { provider?: string; model?: string } {
...
export function apply(ctx: Context, config: Config): void {
   ...
   */
  async function declaredModels(pid: string, llm: any): Promise<any[]> {
    const configured = (llm.listConfigurableProviders?.() ?? []) as any[]
    const entry = configured.find((c: any) => (c?.provider ?? c?.id) === pid)
    if (!entry) return []
    const ns = entry.settingsNs
    if (typeof ns !== 'string' || ns.length === 0) return []
    try {
      const value = (getSettings()?.get?.(ns) as any) ?? {}
      let section = value
      for (const seg of entry.settingsPath ?? []) section = section?.[seg]
  ...
  async function catalog(providerId?: string): Promise<Array<{ provider: string; id: string; name: string; capabilities?: string }>> {
    ...
    if (providerId) {
      ...
      const configured = (llm.listConfigurableProviders?.() ?? []) as any[]
      const ids = [
        ...providers.map((p: any) => p?.id ?? p?.provider ?? p?.name),
        ...configured.map((c: any) => c?.provider ?? c?.id),
      ]
      providerIds = [...new Set(
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    async execute(args: {
      ...
      const parent = exec.agent as any
      if (!parent) throw new Error('subagent_model requires a calling agent (exec.agent was undefined)')

      const provider = args.provider ?? config.defaultProvider ?? parentRoute(parent).provider
      if (!provider) throw new Error('subagent_model: could not resolve an LLM provider — pass `provider` or set config.defaultProvider')
      ...
      const registeredNames: string[] = subagents.list?.() ?? []
      const transportName = registeredNames.includes(transport)
        ? transport
        : registeredNames[0]
      if (!transportName) throw new Error('subagent_model: no ctx.subagents provider is registered')
